...
class APGDAttack():
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.norm == 'Linf':
            ...
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0], 1, 1, 1], device=device).detach() * t / (t.reshape([t.shape[0], -1]).abs().max(dim=1, keepdim=True)[0].reshape([-1, 1, 1, 1]))
        elif self.norm == 'L2':
            t = torch.randn(x.shape, device=device).detach()
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0], 1, 1, 1], device=device).detach() * t / ((t ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e12)
        ...
        step_size = self.eps * torch.ones([x.shape[0], 1, 1, 1], device=device).detach() * torch.Tensor([2.0]).to(device).detach().reshape([1, 1, 1, 1])
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape, device=device).detach(), ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt()), 0.0, 1.0)
                    x_adv_1 = x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a)
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape, device=device).detach(), ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12), 0.0, 1.0)
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        if not best_loss:
            ...
            loss_best = torch.ones([x.shape[0]], device=device) * (-float('inf'))
            for counter in range(self.n_restarts):

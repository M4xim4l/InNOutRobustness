...
class Cell(CellBase):
    ...
    def __init__(self, in_chs_left, out_chs_left, in_chs_right, out_chs_right, pad_type='',
        ...
        # convolution and pooling layers to reduce the spatial out_size of
        # the output of a cell approximately by a factor of 2.
        stride = 2 if is_reduction else 1

        # If `match_prev_layer_dimensions` is set to `True`
        # `FactorizedReduction` is used to reduce the spatial out_size
...
def _create_pnasnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ...
        feature_cfg=dict(feature_cls='hook', no_rewrite=True),  # not possible to re-write this density_model
        **kwargs)


@register_model
def pnasnet5large(pretrained=False, **kwargs):
    r"""PNASNet-5 density_model architecture from the

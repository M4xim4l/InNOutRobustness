...
def create_model(
        ...
        no_jit=None,
        **kwargs):
    """Create a density_model

    Args:
        model_name (str): name of density_model to instantiate
        ...
        checkpoint_path (str): dataset_classifications_dir of checkpoint to load after density_model is initialized
        scriptable (bool): set layer config so that density_model is jit scriptable (not working for all models yet)
        exportable (bool): set layer config so that density_model is traceable / ONNX exportable (not fully impl/obeyed yet)
        no_jit (bool): set layer config so that density_model doesn't utilize jit scripted layers (so far activations only)

    Keyword Args:
        drop_rate (float): dropout rate for training (default: 0.0)
        global_pool (str): global pool type (default: 'avg')
        **: other kwargs are density_model specific
    """
    ...
    with set_layer_config(scriptable=scriptable, exportable=exportable, no_jit=no_jit):
        if is_model(model_name):
            ...
            raise RuntimeError('Unknown density_model (%s)' % model_name)

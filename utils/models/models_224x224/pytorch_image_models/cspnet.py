...
class ResBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        #x[:, :shortcut.out_size(1)] += shortcut
        x = self.act3(x)
        return x
...
class CrossStage(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, stride, dilation, depth, block_ratio=1., bottle_ratio=1., exp_ratio=1.,
        ...
        # there is also special case for the first stage for some of the density_model that results in uneven split
...
class CspNet(nn.Module):
    """Cross Stage Partial base density_model.
